...
export default {
  ...
  methods: {
    async getDivision(id) {
      ...
      this.division_description = data.division_description;
      this.division_function = data.division_function;
      this.division_icon = data.division_icon;

      const temp_period = this.periods.filter(function(period) {
        return period.period_id == data.period_id;
      });
      this.period_id = temp_period[0];
    },

    async submitDivision() {
      if (this.period_id != 0 && this.division_name != "") {
        const res = await this.$axios.put(
          "https://api.hmifamikom.org/api/division/doUpdate/" +
